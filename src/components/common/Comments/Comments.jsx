            ...
            {currentUser && (
                <div className=" shadow-2xl p-3 my-5 overflow-hidden border border-gray-300">
                    <div className=" flex items-center gap-2 mb-5">
                        ...
                        <h3 className=" capitalize">{getUserData?.username}</h3>
                    </div>
                    <textarea 
                        placeholder="What are your thoughts?" 
                        className=" w-full outline-none text-sm border pt-4 px-2 resize-none" 
                        value={comment}
                        onChange={(e) => setComment(e.target.value)}
                    />
                    <div className=" flex items-center justify-end gap-4 mt-[1rem]">
                        <button onClick={() => setComment("")} className=" text-sm">Cancel</button>
                        <button onClick={writeComment} className=" btn !text-xs !bg-green-700 !text-white !rounded-full">Response</button>
                    </div>
                </div>
            )}
        </section>
